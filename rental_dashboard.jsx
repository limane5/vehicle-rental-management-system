...
import { Gauge, Users, Car, TrendingUp, Fuel } from "lucide-react";

// ---------------------------------------------------------------------
// Sample data shaped exactly like the Oracle views from 10_innovation_analytics.sql
// Swap these arrays for a real fetch() against ORDS / APEX / a small
// Flask-Node API in front of VW_REVENUE_BY_BRAND, VW_MONTHLY_REVENUE,
// VW_FLEET_UTILIZATION and VW_TOP_CUSTOMERS.
// ---------------------------------------------------------------------
const REVENUE_BY_BRAND = [
...
export default function RentalDashboard() {
  ...
  const totalRevenue = useMemo(() => REVENUE_BY_BRAND.reduce((s, r) => s + r.TOTAL_REVENUE, 0), []);
  const totalRentals = useMemo(() => REVENUE_BY_BRAND.reduce((s, r) => s + r.TOTAL_RENTALS, 0), []);
  const fleetSize = useMemo(() => FLEET_UTILIZATION.reduce((s, f) => s + f.VEHICLE_COUNT, 0), []);
    ...
    <div style={{ background: palette.bg, minHeight: "100%", fontFamily: "Inter, sans-serif", color: palette.text, padding: "0 0 24px" }}>
      ...
      <div style={{ padding: "24px" }}>
        ...
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 28 }}>
          ...
          <KpiCard icon={Car} label="Fleet size" value={<Odometer value={fleetSize} />} sub={`${FLEET_UTILIZATION.find(f => f.STATUS === "RENTED")?.VEHICLE_COUNT ?? 0} currently rented`} accent={palette.text} />
          <KpiCard icon={Users} label="Top customer" value={TOP_CUSTOMERS[0].CUSTOMER_NAME} sub={<Odometer value={TOP_CUSTOMERS[0].TOTAL_SPEND} prefix="$" />} accent={palette.red} />
        </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 16, marginBottom: 28 }}>
              <div style={{ background: palette.cardSolid, border: `1px solid ${palette.border}`, borderRadius: 4, padding: 18 }}>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={REVENUE_BY_BRAND}>
              ...
              <div style={{ background: palette.cardSolid, border: `1px solid ${palette.border}`, borderRadius: 4, padding: 18, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                ...
                <Gauge360
                  segments={[
                    { label: "AVAILABLE", value: FLEET_UTILIZATION[0].VEHICLE_COUNT, color: palette.teal },
                    { label: "RENTED", value: FLEET_UTILIZATION[1].VEHICLE_COUNT, color: palette.amber },
                    { label: "MAINTENANCE", value: FLEET_UTILIZATION[2].VEHICLE_COUNT, color: palette.red },
                  ]}
                />
                <div style={{ display: "flex", gap: 14, marginTop: 8, flexWrap: "wrap", justifyContent: "center" }}>
                  {FLEET_UTILIZATION.map((f) => (
      ...
      <div style={{ padding: "0 24px", fontSize: 11, color: palette.textMuted }}>
        Sample data shown — connect to VW_REVENUE_BY_BRAND, VW_MONTHLY_REVENUE, VW_FLEET_UTILIZATION and VW_TOP_CUSTOMERS via ORDS, APEX, or a small API layer for live figures.
      </div>
    </div>
  );
}
